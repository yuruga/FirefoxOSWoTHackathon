...
$(function () {
    ...
    function onReadDescriptor(event) {
        ...
        Log.d(TAG, "start writeDescriptor  select_srvc_id ::" + " : " + select_srvc_id.uuid);
        Log.d(TAG, "start writeDescriptor  select_char_id ::" + " : " + select_char_id.uuid);
        Log.d(TAG, "start writeDescriptor  select_descr_id::" + " : " + select_descr_id.uuid);
        ...
        var confirm = {
            ...
            ok: function (value) {
                ...
                if (value) {
                    var len = value.length;
                    Log.d(TAG, value + ":" + len);
                    bleManager.writeDescriptor(conn_id, select_srvc_id, select_char_id, select_descr_id, 2, len, 0, value);
                }
                Dialog.hidden();
            }
        };
    ...
    $('#des_notify').on('click', function() {
        notify(isnotify);
    });

    function notify(is_notify) {
        if (is_notify) {
            isnotify = false;
            $('#des_notify').html("Start Notify");
            bleManager.deregisterForNotification(client_if, bd_addr, select_srvc_id, select_char_id);
        } else {
            isnotify = true;
            var v ="0100"//decodeURI("%00")+decodeURI("%01");
            var len = 4;
            var wt = 2;
            Log.d(TAG, v + ":" + len, encodeURIComponent(v));
            bleManager.registerForNotification(client_if, bd_addr, select_srvc_id, select_char_id);
            bleManager.writeDescriptor(conn_id, select_srvc_id, select_char_id, select_descr_id, wt, len, 0, v);
            $('#des_notify').html("Stop Notify");
        }
    }
